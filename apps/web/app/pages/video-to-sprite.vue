...
function buildOutputName(input: string, format: VideoToSpriteOptions['output_format']): string {
  const dot = input.lastIndexOf('.')
  const stem = dot > 0 ? input.slice(0, dot) : input
  return `${stem}-sprite.${format}`
}
...
const sendChromaKey = computed(() => options.value.chroma_key)
// Expose a payload that omits chroma_* if disabled.
function buildPayload(): VideoToSpriteOptions {
  const out: VideoToSpriteOptions = { ...options.value }
  // The api-client checks options.chroma_key flag — pass through.
  return out
}
...
async function run() {
  ...
  lines.value.push({
    id: logId++,
    text: `Building sprite for ${items.value.length} video via ${api.mode} API...`,
    status: 'info',
  })
  ...
  for (const item of items.value) {
    ...
    try {
      const result = await api.client.processVideoToSpriteUpload(
        item.file,
        buildPayload(),
      )
      ...
      item.fps = result.fps
      item.frameSize = result.frameSize
      item.status = 'done'
      item.durationMs = Math.round(performance.now() - t0)
      lines.value.push({
        id: logId++,
        text: `[${current.value}/${total.value}] ${item.file.name} → ${outName} · ${result.frameCount ?? '?'} frames @ ${result.fps ?? options.value.fps}fps (${item.durationMs}ms)`,
        status: 'ok',
      })
    } catch (err) {
      item.status = 'error'
  ...
  lines.value.push({
    id: logId++,
    text: `Done — ${doneCount.value} sprite, ${errorCount.value} failed`,
...
<template>
  <ToolHeader
    eyebrow="Animation"
    title="Video → Sprite"
    description="Ekstrak frame dari MP4/MOV/WebM dan jahit jadi sprite sheet horizontal. Optional chroma key per frame."
  />
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
      <div class="space-y-6">
        <SettingsPanel
          ...
          :subtitle="
            items.length === 0
              ? 'Drop video (MP4/MOV/WebM) atau pilih dari komputer.'
              : `${items.length} video siap diproses`
          "
        >
          ...
          <FileDropZone
            ...
            :multiple="true"
            hint="MP4, MOV, WebM — pemrosesan butuh ffmpeg"
            @files="handleFiles"
          />

          <ul
            v-if="items.length > 0"
            class="-m-1 mt-1 grid gap-2"
          >
            <li
              v-for="item in items"
              :key="item.id"
            ...
            >
              <div class="relative flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-md border bg-muted/40">
                <img
                  v-if="item.outputUrl"
                  :src="item.outputUrl"
                  :alt="`Sprite for ${item.file.name}`"
                  class="absolute inset-0 size-full object-contain p-0.5"
                  loading="lazy"
                />
                <Film v-else class="size-5 text-muted-foreground/70" />
                <div
                ...
                <div class="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground">
                  ...
                  <template v-if="item.status === 'done'">
                    ...
                    <span class="inline-flex items-center gap-0.5 text-success">
                      ...
                      {{ item.frameCount }} frames @ {{ item.fps }}fps · {{ item.durationMs }}ms
                    </span>
                  </template>
              ...
              <div class="flex items-center">
                ...
                >
                  ...
                  <span class="hidden sm:inline">Sprite</span>
                </button>
                <button
                  type="button"
        ...
        <SettingsPanel title="Sprite settings" subtitle="Atur sampling dan format output.">
          <div class="grid gap-5 sm:grid-cols-2">
            <div class="space-y-2">
              <label for="vid_fps" class="flex items-center justify-between text-sm font-medium">
                Target FPS
                <span class="font-mono text-[11px] tabular-nums text-muted-foreground">{{ options.fps }}</span>
              </label>
              <input id="vid_fps" v-model.number="options.fps" type="range" min="1" max="30" step="1" class="w-full accent-primary" />
            ...
            <div class="space-y-2">
              <label for="vid_size" class="flex items-center justify-between text-sm font-medium">
                Frame size
                <span class="font-mono text-[11px] tabular-nums text-muted-foreground">{{ options.frame_size }}px</span>
              </label>
              <input id="vid_size" v-model.number="options.frame_size" type="range" min="64" max="1024" step="32" class="w-full accent-primary" />
              <div class="flex justify-between text-[10px] text-muted-foreground">
                <span>64</span>
                <span>1024</span>
              </div>
            ...
            <div class="space-y-2">
              <label for="vid_format" class="text-sm font-medium">Output format</label>
              <select
                id="vid_format"
            ...
            <div v-if="options.output_format === 'webp'" class="space-y-2 animate-fade-in">
              <label for="vid_quality" class="flex items-center justify-between text-sm font-medium">
                WebP quality
                <span class="font-mono text-[11px] tabular-nums text-muted-foreground">{{ options.webp_quality }}</span>
              </label>
              <input id="vid_quality" v-model.number="options.webp_quality" type="range" min="0" max="100" step="1" class="w-full accent-primary" />
            </div>
          </div>
        </SettingsPanel>
        ...
        <SettingsPanel
          title="Chroma key"
          subtitle="Optional — apply BG remove pada setiap frame sebelum stitch."
        >
          <template #actions>
            <label class="inline-flex cursor-pointer items-center gap-2">
              <span class="text-xs text-muted-foreground">{{ sendChromaKey ? 'Aktif' : 'Off' }}</span>
              <input
          ...
          <div v-if="options.chroma_key" class="grid gap-5 sm:grid-cols-2 animate-fade-in">
            <div class="space-y-2">
              <label for="ck_target" class="flex items-center justify-between text-sm font-medium">
                Target color
                <span class="font-mono text-[11px] uppercase text-muted-foreground">{{ options.chroma_target }}</span>
              </label>
              <div class="flex items-center gap-2">
                <input
                  ...
                  aria-label="Target chroma color"
                />
                <input
                  v-model="options.chroma_target"
                  type="text"
            ...
            <div class="space-y-2">
              <label for="ck_fuzz" class="flex items-center justify-between text-sm font-medium">
                Fuzz tolerance
                <span class="font-mono text-[11px] tabular-nums text-muted-foreground">{{ options.chroma_fuzz.toFixed(2) }}</span>
              </label>
              <input id="ck_fuzz" v-model.number="options.chroma_fuzz" type="range" min="0" max="1" step="0.01" class="w-full accent-primary" />
            </div>
            <div class="space-y-2">
              <label for="ck_erode" class="flex items-center justify-between text-sm font-medium">
                Erode iterations
                <span class="font-mono text-[11px] tabular-nums text-muted-foreground">{{ options.chroma_erode }}</span>
              </label>
              <input id="ck_erode" v-model.number="options.chroma_erode" type="range" min="0" max="5" step="1" class="w-full accent-primary" />
            </div>
            <label
              class="flex cursor-pointer items-center gap-2.5 rounded-md border bg-muted/30 px-3 py-2.5 hover:bg-muted/50 transition-colors"
            >
              <input
                ...
                class="size-4 rounded border-input accent-primary"
              />
              <span class="text-sm font-medium">Despill green channel</span>
            </label>
          </div>
        </SettingsPanel>
      ...
      <aside class="space-y-5 lg:sticky lg:top-20 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto lg:pr-1 scroll-thin">
        <SettingsPanel title="Sprite preview">
          <div class="rounded-lg border bg-checker p-2 flex items-center justify-center min-h-[8rem] overflow-hidden">
            <img
              v-if="activeItem?.outputUrl"
              :src="activeItem.outputUrl"
              alt="Sprite sheet preview"
              class="max-h-32 max-w-full object-contain animate-fade-in"
              loading="lazy"
            />
            <div v-else class="flex flex-col items-center gap-1 text-muted-foreground/70">
              <Film class="size-5" aria-hidden="true" />
              <span class="text-[11px]">Belum ada sprite</span>
            </div>
          </div>
          <dl
            v-if="activeItem?.status === 'done'"
            class="mt-3 grid grid-cols-3 divide-x divide-border rounded-md border bg-muted/30 text-center text-xs"
          >
            <div class="px-2 py-2">
              <dt class="text-[10px] uppercase tracking-wide text-muted-foreground">Frames</dt>
              <dd class="font-mono font-semibold tabular-nums">{{ activeItem.frameCount ?? '—' }}</dd>
            </div>
            <div class="px-2 py-2">
              <dt class="text-[10px] uppercase tracking-wide text-muted-foreground">FPS</dt>
              <dd class="font-mono font-semibold tabular-nums">{{ activeItem.fps ?? '—' }}</dd>
            </div>
            <div class="px-2 py-2">
              <dt class="text-[10px] uppercase tracking-wide text-muted-foreground">Size</dt>
              <dd class="font-mono font-semibold tabular-nums">{{ activeItem.frameSize ?? '—' }}px</dd>
            </div>
          </dl>
          <p
            ...
            <span>
              {{
                running
                  ...
                  : items.length === 0
                    ? 'Build sprite'
                    : `Build ${items.length} sprite${items.length === 1 ? '' : 's'}`
              }}
            </span>
            <kbd
