...
import {
  DEFAULT_ANIM_PREVIEW_OPTIONS,
  type AnimPreviewOptions,
} from '~/types/pixiekit'

useHead({ title: 'Anim Preview — Pixiekit' })
...
const options = ref<AnimPreviewOptions>({ ...DEFAULT_ANIM_PREVIEW_OPTIONS })
...
const presets = useToolPreset<AnimPreviewOptions>('anim-preview', [
  { name: 'Standard GIF', options: { ...DEFAULT_ANIM_PREVIEW_OPTIONS } },
  { name: 'Smooth MP4', options: { fps: 24, format: 'mp4', loop_anim: true, upscale: 1 } },
])
...
function buildOutputName(input: string, format: AnimPreviewOptions['format']): string {
...
async function run() {
  ...
  for (const item of items.value) {
    ...
    try {
      const blob = await api.client.processAnimPreviewUpload(item.file, options.value)
      const outName = buildOutputName(item.file.name, options.value.format)
      if (item.outputUrl) URL.revokeObjectURL(item.outputUrl)
      item.outputBlob = blob
      item.outputUrl = URL.createObjectURL(blob)
      item.outputName = outName
      item.status = 'done'
      item.format = options.value.format
...
<template>
  <ToolHeader
    eyebrow="Animation"
    title="Anim Preview"
    description="Konversi sprite sheet atau folder frame menjadi animasi (GIF, MP4, WebM)."
  />
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
      ...
      <div class="space-y-6">
        <SettingsPanel
          ...
          :subtitle="
            items.length === 0
              ? 'Drag & drop sprite sheet untuk diproses.'
          ...
          >
            ...
            >
              <div class="relative size-14 shrink-0 overflow-hidden rounded-md border bg-checker">
                <img
                  v-if="!item.outputUrl || item.format === 'gif'"
        ...
        <SettingsPanel title="Animation" subtitle="Atur frame rate dan output format.">
          <div class="grid gap-5 sm:grid-cols-2">
            <div class="space-y-2">
              <label for="fps" class="flex items-center justify-between text-sm font-medium">
                FPS
            ...
            <div class="space-y-2">
              <label for="format" class="text-sm font-medium">Output format</label>
              <select
                id="format"
            ...
            <div class="space-y-2">
              <label for="upscale" class="flex items-center justify-between text-sm font-medium">
                Upscale
                <span class="font-mono text-[11px] tabular-nums text-muted-foreground">
                  {{ options.upscale }}x
                </span>
              ...
              <input
                id="upscale"
                v-model.number="options.upscale"
                type="range"
                min="1"
                max="8"
                step="1"
            ...
            <div class="space-y-2">
              <label for="frame_size" class="text-sm font-medium">Frame Size (Override)</label>
              <input
                id="frame_size"
                v-model.number="options.frame_size"
                type="number"
                placeholder="Auto"
                class="h-10 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              />
            ...
            <label
              class="group/check flex cursor-pointer items-center gap-2.5 rounded-md border bg-muted/30 px-3 py-2.5 hover:bg-muted/50 transition-colors sm:col-span-2"
            >
              <input
                v-model="options.loop_anim"
                type="checkbox"
                class="size-4 rounded border-input accent-primary"
              />
              <span class="flex-1">
                <span class="block text-sm font-medium">Loop Animation</span>
      ...
      <aside class="space-y-5 lg:sticky lg:top-20 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto lg:pr-1 scroll-thin">
        <SettingsPanel title="Preview" :flush="false">
          <div class="relative aspect-square overflow-hidden rounded-lg border bg-checker">
            <template v-if="activeItem?.outputUrl">
              <img
                v-if="activeItem.format === 'gif'"
            ...
            <div v-else class="flex h-full flex-col items-center justify-center p-8 text-center">
              <Play class="mb-2 size-8 text-muted-foreground/30" />
              <p class="text-xs text-muted-foreground italic">
                Process file untuk melihat preview animasi.
              </p>
            </div>
          </div>
          <p
